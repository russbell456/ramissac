...
class AlmacenPrestamoService:
    ...
    def registrar_prestamo_desde_qr(
        ...
        for item in data.items:
            ...
            if not art:
                raise ValueError(
                    f"Artículo no encontrado: {item.articulo_id}"
                )

            if art.stock_actual < item.cantidad:
                raise ValueError(
                    f"Sin stock suficiente para {art.nombre}"
                )
        ...
        for det in prestamo.detalles:
            ...
            articulo = self.db.query(
                AlmacenArticulo
            ).get(det.articulo_id)

            detalles_schema.append(
                PrestamoDetalleSchema(
                    id=det.id,
    ...
    def _map_prestamo_to_schema(
        ...
        detalles = [
            PrestamoDetalleSchema(
                ...
                articulo_tipo=(
                    det.articulo.tipo.value
                    if det.articulo
                    else "unknown"
                ),
                articulo_unidad=(
                    det.articulo.unidad_medida
                    if det.articulo
                    else ""
                )
            )
